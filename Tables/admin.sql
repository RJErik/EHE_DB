...
CREATE OR REPLACE FUNCTION trg_admin_set_audit_fields()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_admin_set_audit_fields
BEFORE UPDATE ON admin
FOR EACH ROW
EXECUTE PROCEDURE trg_admin_set_audit_fields();

CREATE OR REPLACE FUNCTION trg_admin_audit_log_history()
RETURNS TRIGGER AS $$
DECLARE
    dml_type CHAR(1);
...
CREATE TRIGGER trg_admin_audit_log_history
AFTER INSERT OR UPDATE OR DELETE ON admin
FOR EACH ROW
EXECUTE PROCEDURE trg_admin_audit_log_history();
