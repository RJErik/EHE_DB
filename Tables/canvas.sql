...
CREATE OR REPLACE FUNCTION trg_canvas_set_audit_fields()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_canvas_set_audit_fields
BEFORE UPDATE ON canvas
FOR EACH ROW
EXECUTE PROCEDURE trg_canvas_set_audit_fields();

CREATE OR REPLACE FUNCTION trg_canvas_audit_log_history()
RETURNS TRIGGER AS $$
DECLARE
    dml_type CHAR(1);
...
CREATE TRIGGER trg_canvas_audit_log_history
AFTER INSERT OR UPDATE OR DELETE ON canvas
FOR EACH ROW
EXECUTE PROCEDURE trg_canvas_audit_log_history();
