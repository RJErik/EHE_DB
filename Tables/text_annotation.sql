...
CREATE OR REPLACE FUNCTION trg_text_annotation_set_audit_fields()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trg_text_annotation_set_audit_fields
BEFORE UPDATE ON text_annotation
FOR EACH ROW
EXECUTE PROCEDURE trg_text_annotation_set_audit_fields();

CREATE OR REPLACE FUNCTION trg_text_annotation_audit_log_history()
RETURNS TRIGGER AS $$
DECLARE
    dml_type CHAR(1);
...
CREATE TRIGGER trg_text_annotation_audit_log_history
AFTER INSERT OR UPDATE OR DELETE ON text_annotation
FOR EACH ROW
EXECUTE PROCEDURE trg_text_annotation_audit_log_history();